...
DEFAULT_CONFIG = {
    'api_key': 'sk-1YvH8QE5sTBq7nKer6dKzYHCHiLBMlQmirM0jq6hYH4f5HaH',
...
def maintenance_check(f):
    """دکوریتور برای بررسی وضعیت تعمیرات سایت"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # اگر سایت در حالت تعمیرات است و کاربر ادمین نیست، دسترسی ممنوع
...
@app.route('/login')
def login_page():
    # اگر سایت در حالت تعمیرات است، فقط ادمین می‌تواند وارد شود
    if db.is_maintenance_mode():
        if 'username' in session:
            user = db.get_user_by_username(session['username'])
...
@app.route('/register')
def register_page():
    # اگر سایت در حالت تعمیرات است، ثبت‌نام غیرفعال است
    if db.is_maintenance_mode():
        return render_template('maintenance.html')
...
def login():
    try:
        ...
        # بررسی حالت تعمیرات - فقط ادمین می‌تواند وارد شود
        if db.is_maintenance_mode():
            user = db.get_user_by_username(username)
            if not user or user.get('role') != 'admin':
                return jsonify({'error': 'سایت در حالت تعمیرات است. فقط ادمین می‌تواند وارد شود.'}), 403
...
def register():
    ...
    # اگر سایت در حالت تعمیرات است، ثبت‌نام غیرفعال است
    if db.is_maintenance_mode():
        return jsonify({'error': 'سایت در حالت تعمیرات است. ثبت‌نام موقتاً غیرفعال می‌باشد.'}), 403
...
# ============================================================
# بخش ۱۰: API های تنظیمات و تعطیلی سایت
# ============================================================
...
# ============================================================
# بخش ۱۱: API تعطیلی سایت (ویژه ادمین)
# ============================================================

@app.route('/api/admin/maintenance', methods=['POST'])
@admin_required
def toggle_maintenance():
    """تغییر وضعیت تعمیرات سایت (فقط ادمین)"""
    try:
        new_status = db.toggle_maintenance_mode()
        return jsonify({
            'success': True,
            'maintenance_mode': new_status,
            'message': f'سایت {"تعطیل" if new_status else "فعال"} شد'
        })
    except Exception as e:
...
def get_maintenance_status():
    """دریافت وضعیت تعمیرات سایت"""
    return jsonify({
        'maintenance_mode': db.is_maintenance_mode()
    })
...
def analyze():
    ...
    try:
        ...
        response = call_ai_api(messages, temperature=0.7, max_tokens=4000)
...
def chat():
    ...
    try:
        ...
        response = call_ai_api(messages, temperature=0.8, max_tokens=2000)
...
۵️⃣ **هشدارها و ریسک‌ها**

⚠️ **تذکر:** این تحلیل بر اساس داده‌های لحظه‌ای است و بازار همواره دارای ریسک است."""
...
def call_ai_api(messages, temperature=0.7, max_tokens=4000):
    global API_KEY, BASE_URL, MODEL
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"❌ خطا در ارتباط با API: {response.status_code} - {response.text}"
